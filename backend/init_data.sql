...
-- 기본 카테고리 데이터 (Ingress 경로로 저장)
INSERT INTO category (name, display_name, icon) VALUES
('food',        '식품',         '/minio/category/food.jpeg'),
('living',      '생활용품',      '/minio/category/living.jpeg'),
('beauty',      '뷰티',         '/minio/category/beauty.jpeg'),
('interior',    '인테리어',      '/minio/category/interior.jpeg'),
('electronics', '가전/디지털',    '/minio/category/electronics.jpeg'),
('kitchen',     '주방용품',      '/minio/category/kitchen.jpeg'),
('pet',         '반려동물',      '/minio/category/pet.jpeg'),
('sports',      '스포츠',        '/minio/category/sports.jpeg'),
('books',       '도서/음반',     '/minio/category/books.jpeg'),
('health',      '헬스/건강',     '/minio/category/health.jpeg');
-- ('food',        '식품',         'http://192.168.80.90:30222/category/food.jpeg'),
-- ('living',      '생활용품',      'http://192.168.80.90:30222/category/living.jpeg'),
-- ('beauty',      '뷰티',         'http://192.168.80.90:30222/category/beauty.jpeg'),
-- ('interior',    '인테리어',      'http://192.168.80.90:30222/category/interior.jpeg'),
-- ('electronics', '가전/디지털',    'http://192.168.80.90:30222/category/electronics.jpeg'),
-- ('kitchen',     '주방용품',      'http://192.168.80.90:30222/category/kitchen.jpeg'),
-- ('pet',         '반려동물',      'http://192.168.80.90:30222/category/pet.jpeg'),
-- ('sports',      '스포츠',        'http://192.168.80.90:30222/category/sports.jpeg'),
-- ('books',       '도서/음반',     'http://192.168.80.90:30222/category/books.jpeg'),
-- ('health',      '헬스/건강',     'http://192.168.80.90:30222/category/health.jpeg');
-- 3) products
...
-- 9) outbox_events (Transactional Outbox Pattern)
...
-- 10) processed_events (Consumer 멱등성 - 중복 처리 방지)
...
-- 11) stock_history (재고 변동 이력 - 감사 로그)
...
-- 12) daily_sales (일별 매출 집계)
CREATE TABLE daily_sales (
  date             DATE PRIMARY KEY,
  total_orders     INT NOT NULL DEFAULT 0,
  total_revenue    BIGINT NOT NULL DEFAULT 0,
  -- unique_customers 제거: 같은 유저가 하루에 여러 주문 시 중복 카운트 문제
  -- 필요하면 쿼리로 계산: SELECT date, COUNT(DISTINCT user_id) FROM orders GROUP BY date
  updated_at       TIMESTAMPTZ NOT NULL DEFAULT now()
);


-- 13) product_daily_stats (상품별 일일 통계)
...
('곰표 우유 식빵 660g', 5000, 19, '곰표', 1, '/minio/products/1_gompyo_milk.jpeg', 0, true, 99999),
('신라면 멀티팩 5개입', 5500, 19, '농심', 1, '/minio/products/2_shin_ramyun.jpeg', 0, true, 99999),
('제주 감귤 3kg', 25000, 21, '제주농협', 1, '/minio/products/3_3kg_jeju.jpeg', 0, true, 99999);

-- 생활용품 (category_id = 2)
INSERT INTO products (name, original_price, discount_percent, brand, category_id, image, view_count, free_shipping, stock) VALUES
('다우니 섬유유연제 3.2L', 19900, 20, '다우니', 2, '/minio/products/4_downy_fabric.jpeg', 0, true, 99999),
('코멧 3겹 화장지 30롤', 18900, 0, '코멧', 2, '/minio/products/5_comet_3ply.jpeg', 0, true, 99999),
('페리오 치약 클리닉 3개', 11900, 25, 'LG생활건강', 2, '/minio/products/6_perio_toothpaste.jpeg', 0, true, 99999);

-- 뷰티 (category_id = 3)
INSERT INTO products (name, original_price, discount_percent, brand, category_id, image, view_count, free_shipping, stock) VALUES
('아이오페 레티놀 세럼 30ml', 65000, 20, 'IOPE', 3, '/minio/products/7_iope_retinol.jpeg', 0, true, 99999),
('라로슈포제 선크림 50ml', 24900, 0, '라로슈포제', 3, '/minio/products/8_la_rocheposay.jpeg', 0, true, 99999);

-- 홈인테리어 (category_id = 4)
INSERT INTO products (name, original_price, discount_percent, brand, category_id, image, view_count, free_shipping, stock) VALUES
('이케아 LED 스탠드', 29900, 0, 'IKEA', 4, '/minio/products/9_ikea_led.jpeg', 0, false, 99999),
('무인양품 소파쿠션', 25000, 20, 'MUJI', 4, '/minio/products/10_muji_sofa.jpeg', 0, true, 99999);

-- 가전디지털 (category_id = 5)
INSERT INTO products (name, original_price, discount_percent, brand, category_id, image, view_count, free_shipping, stock) VALUES
('애플 맥북 에어 M3', 1690000, 6, 'Apple', 5, '/minio/products/11_apple_macbook.jpeg', 0, true, 99999),
('삼성 갤럭시 버즈 FE', 89000, 0, '삼성', 5, '/minio/products/12_samsung_galaxy.jpeg', 0, true, 99999);

-- 주방용품 (category_id = 6)
INSERT INTO products (name, original_price, discount_percent, brand, category_id, image, view_count, free_shipping, stock) VALUES
('휘슬러 압력솥 6L', 350000, 17, '휘슬러', 6, '/minio/products/13_fissler_pressure.jpeg', 0, true, 99999),
('테팔 프라이팬 28cm', 39900, 0, '테팔', 6, '/minio/products/14_tefal_28cm.jpeg', 0, true, 99999),
('코렐 접시세트 16P', 99000, 20, '코렐', 6, '/minio/products/15_corelle_plate.jpeg', 0, true, 99999);

-- 반려동물 (category_id = 7)
INSERT INTO products (name, original_price, discount_percent, brand, category_id, image, view_count, free_shipping, stock) VALUES
('로얄캐닌 사료 3kg', 39900, 18, '로얄캐닌', 7, '/minio/products/16_royal_canin.jpeg', 0, true, 99999),
('하림펫푸드 간식 300g', 8900, 0, '하림', 7, '/minio/products/17_harim_pet.jpeg', 0, true, 99999),
('캣타워 대형 180cm', 120000, 26, '펫모닝', 7, '/minio/products/18_large_cat.jpeg', 0, false, 99999);

-- 스포츠/레저 (category_id = 8)
INSERT INTO products (name, original_price, discount_percent, brand, category_id, image, view_count, free_shipping, stock) VALUES
('나이키 에어맥스 270', 199000, 15, 'Nike', 8, '/minio/products/19_nike_air.jpeg', 0, true, 99999),
('요가매트 NBR 10mm', 19900, 0, '에바', 8, '/minio/products/20_yoga_mat.jpeg', 0, true, 99999),
('캠핑 텐트 4인용', 200000, 21, '코베아', 8, '/minio/products/21_camping_tent.jpeg', 0, true, 99999);

-- 도서/음반 (category_id = 9)
INSERT INTO products (name, original_price, discount_percent, brand, category_id, image, view_count, free_shipping, stock) VALUES
('트렌드 코리아 2025', 19000, 10, '미래의창', 9, '/minio/products/22_trend_korea.jpeg', 0, true, 99999),
('아웃라이어', 15300, 0, '김영사', 9, '/minio/products/23_outliers.jpeg', 0, true, 99999),
('BTS 앨범 정규 4집', 50000, 10, 'HYBE', 9, '/minio/products/24_bts_4th.jpeg', 0, true, 99999);

-- 헬스/건강 (category_id = 10)
INSERT INTO products (name, original_price, discount_percent, brand, category_id, image, view_count, free_shipping, stock) VALUES
('종근당 비타민C 1000', 25000, 20, '종근당', 10, '/minio/products/25_chong_kun.jpeg', 0, true, 99999),
('락토핏 유산균 60포', 23900, 0, '종근당', 10, '/minio/products/26_lactofit_lactic.jpeg', 0, true, 99999),
('홍삼정 에브리타임', 110000, 19, '정관장', 10, '/minio/products/27_hongsamjeong_everytime.jpeg', 0, true, 99999);
